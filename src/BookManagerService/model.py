...
class _BookReview:
        ...
        try:
            ...
            if not self.try_reconnect():
                ...
                raise CouldNotConnectToDatabase(
                    "Could not connect to database") from e
        finally:
            cur = self.connection.cursor()

        cur.execute("SELECT * FROM read_books WHERE user_id = %s;",
                    (args.user_id,))
        rows = cur.fetchall()
        cur.close()
        return rows
    ...
    def get_book_review(self, args: BookReviewQuerySchema) -> BookReviewSchema:
        ...
        cur.execute(
            "SELECT * FROM read_books WHERE id = %s AND user_id = %s;", (args.id, args.user_id))
        row = cur.fetchone()
        cur.close()
        if row is None:
    ...
    def create_book_review(self, book_review: BookReviewSchema) -> BookReviewSchema:
        ...
        cur.execute("INSERT INTO read_books (user_id, title, review, rating, time_spent) VALUES (%s, %s, %s, %s, %s) RETURNING *;",
                    (book_review.user_id, book_review.title, book_review.review, book_review.rating, book_review.time_spent))
        row = cur.fetchone()
    ...
    def update_book_review(self, book_review: BookReviewSchema) -> BookReviewSchema:
        ...
        cur.execute("UPDATE read_books SET title = %s, review = %s, rating = %s, time_spent = %s WHERE id = %s AND user_id = %s RETURNING *;",
                    (book_review.title, book_review.review, book_review.rating, book_review.time_spent, book_review.id, book_review.user_id))
        row = cur.fetchone()
        self.connection.commit()
        cur.close()
        if row is None:
    ...
    def delete_book_review(self, args: BookReviewQuerySchema) -> BookReviewSchema:
        ...
        cur.execute(
            "DELETE FROM read_books WHERE id = %s AND user_id = %s RETURNING *;", (args.id, args.user_id))
        row = cur.fetchone()
        cur.close()
        if row is None:
            raise BookReviewNotFound("Book review not found")
