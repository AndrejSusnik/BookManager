...
class CustomConfigManager:
        ...
        if useEtcd and (ectd_config is None):
            ...
            try:
                self.etcd_client = etcd.Client(
                    host=ectd_config.host, port=ectd_config.port) if useEtcd else None
    ...
    def load_configurations(self):
        ...
        try:
            with open(self.path + 'config.yaml') as f:
                self.yaml_variables = self.__parse_yaml(
    ...
    def check_etcd(self, key: str, for_service: str = "") -> bool:
        try:
            val = self.etcd_client.read(
                self._get_etcd_path(key, for_service)).value
            return True, val
